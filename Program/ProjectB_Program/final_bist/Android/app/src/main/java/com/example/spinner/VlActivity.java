...
public class VlActivity extends AppCompatActivity implements WebSocketMessageListener {
    ...
    // private List<List<Entry>> entries = new ArrayList<>(); //  // old implementation of graph
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // create the return button (the listener to "VL53L0X Telemetry" button)
    ...
    protected void onStart() {
        ...
        // ID=4 (changeActivity), Dir=0 (stop), Mode=0 (Off), Value=3 (VL)
        WebSocketManager.sendBinaryCommand((byte)4, (byte)0, (byte)0, (byte)3); //send message that the activity has switched
    }
    ...
    public void onBinaryDataReceived(byte type, float timestamp_ms , int distanceLuna, int strengthLuna, float temperatureLuna, float mpuAngle, float mpuTotGyro, float mpuTotAcc, float vl1, float vl2, float vl3, int spinCount,  byte currentTopDir, byte currentHammerDir, byte currentSystemMode, byte currentTopMode, byte currentHammerMode) {
        ...
        if (true/*packetCounter % 2 == 0*/) {
            ...
            if (type == 3 && relativeTimeSec >= 0) { // type = 3 for VL
                ...
                runOnUiThread(() -> {
                    ...
                    if (!Float.isNaN(vl1) && !Float.isInfinite(vl1)) updateGraph(relativeTimeSec, vl1, 0, 0f, 120f);
                    if (!Float.isNaN(vl2) && !Float.isInfinite(vl2)) updateGraph(relativeTimeSec, vl2, 1, 0f, 120f);
                    if (!Float.isNaN(vl3) && !Float.isInfinite(vl3)) updateGraph(relativeTimeSec, vl3, 2, 0f, 120f);
    ...
    public void onOpenWebSocket() {
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                Toast.makeText(VlActivity.this, wsConnected, Toast.LENGTH_SHORT).show();
                // WebSocket is connected again, so clear the red error state.
                updateSpinningStatusUI(MainClass.top_is_spinning);
                for (LineChart chart : mcharts) {
                    chart.setNoDataText("Waiting for VLs data ...");
                    chart.invalidate();
                }
                android.util.Log.d("WS_DEBUG", wsConnected + " waiting for VLs data ...");
            }
        });
    }
    ...
    public void onClosedWebSocket(String reason) {
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                Toast.makeText(VlActivity.this, wsClosed, Toast.LENGTH_LONG).show();
                isTopSpinning.setText("Disconnected");
                isTopSpinning.setTextColor(ContextCompat.getColor(VlActivity.this, R.color.red));
    ...
    public void onFailureWebSocket(String message) {
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                String VlFailWSMsg = "WS Connection Failed while in VL Screen: " + message;
                String wsFailed= "WS Failed!";
                Toast.makeText(VlActivity.this,wsFailed, Toast.LENGTH_LONG).show();
                android.util.Log.e("WS_DEBUG", VlFailWSMsg);
            }
        });
    }
