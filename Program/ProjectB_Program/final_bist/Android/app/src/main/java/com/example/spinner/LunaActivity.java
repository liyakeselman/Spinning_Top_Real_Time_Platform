...
public class LunaActivity extends AppCompatActivity implements WebSocketMessageListener {
    ...
    // function to initial design - to call only from onCreate
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // create the return button (the listener to "TF-Luna Telemetry" button)
        returnToChoice.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(LunaActivity.this, ChoiceActivity.class);
    ...
    protected void onStart() {
        ...
        // ID=4 (changeActivity), Dir=0 (stop), Mode=1 (Off), Value=1 (LUNA)
        WebSocketManager.sendBinaryCommand((byte)4, (byte)0, (byte)0, (byte)1); //send message that the activity has switched
    }
    ...
    public void onBinaryDataReceived(byte type, float timestamp_ms, int distanceLuna, int strengthLuna, float temperatureLuna, float mpuAngle, float mpuTotGyro,float mpuTotAcc, float vl1, float vl2, float vl3, int spinCount, byte currentTopDir, byte currentHammerDir, byte currentSystemMode, byte currentTopMode, byte currentHammerMode) {
        ...
        if (true/*packetCounter % 2 == 0*/) {
            ...
            if (sessionStartTime == -1.0) sessionStartTime = time_sec;
            ...
            if (type == 1 && relativeTimeSec >= 0) {  // type = 1 for LUNA
                ...
                runOnUiThread(() -> {
                    ...
                    if (!Float.isNaN(distanceLuna) && !Float.isInfinite(distanceLuna))
                        updateGraph(relativeTimeSec, distanceLuna, 0, 0f, 200f);
                    if (!Float.isNaN(strengthLuna) && !Float.isInfinite(strengthLuna))
                        updateGraph(relativeTimeSec, strengthLuna, 1, 0f, 2000f); // if strength is below 100 the distance not reliable
                    if (!Float.isNaN(temperatureLuna) && !Float.isInfinite(temperatureLuna))
                        updateGraph(relativeTimeSec, strengthLuna, 2, 0f, 60f);
    ...
    public void onOpenWebSocket() {
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                Toast.makeText(LunaActivity.this, wsConnected, Toast.LENGTH_SHORT).show();
                // WebSocket is connected again, so clear the red error state.
                updateSpinningStatusUI(MainClass.top_is_spinning);
                for (LineChart chart : mcharts) {
                    chart.setNoDataText("Waiting for Luna data ...");
                    chart.invalidate();
                }
                android.util.Log.d("WS_DEBUG", wsConnected + " waiting for Luna data ...");
            }
        });
    }
    ...
    public void onClosedWebSocket(String reason) {
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                Toast.makeText(LunaActivity.this, wsClosed, Toast.LENGTH_LONG).show();
                isTopSpinning.setText("Disconnected");
                isTopSpinning.setTextColor(ContextCompat.getColor(LunaActivity.this, R.color.red));
    ...
    public void onFailureWebSocket(String message) {
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                String LunaFailWSMsg = "WS Connection Failed while in Luna Screen: " + message;
                String wsFailed= "WS Failed!";
                Toast.makeText(LunaActivity.this,wsFailed, Toast.LENGTH_LONG).show();
                android.util.Log.e("WS_DEBUG", LunaFailWSMsg);
            }
        });
    }
